# =============================================================================
# LangGraph 多 Agent 股票分析系统
# 核心架构：主 Agent 路由 + 5 个专业化子 Agent
# =============================================================================
...
class AgentState(TypedDict):
    """
    LangGraph 状态定义
    - messages: 对话历史（使用 add_messages 自动追加新消息）
    - dialog_state: 当前对话阶段（用于跟踪 Agent 切换）
    """
    messages: Annotated[list[AnyMessage], add_messages]
    dialog_state: Annotated[
        list[
            Literal[
                "assistant",      # 主 Agent 阶段
                "full_scan",      # 股票扫描阶段
                "full_analysis",  # 全面分析阶段
                "chart_analysis", # 图表分析阶段
                "determine_risk", # 风险管理阶段
                "gainers_losers", # 涨跌幅榜阶段
            ]
        ],
        update_dialog_stack,
    ]
...
# =============================================================================
# 路由器类：主 Agent 使用这些工具来委托任务给专业化子 Agent
# 关键设计：这些类不是真正的工具，而是触发 Agent 切换的"信号"
# =============================================================================
...
class Assistant:
    def __init__(self, runnable: Runnable):
        self.runnable = runnable  # 工具链

    def __call__(self, state: AgentState, config: RunnableConfig):
        while True:
            result = self.runnable.invoke(state)  # 调用工具链
...
# =============================================================================
# Agent 工厂函数：每个函数创建一个专业化的 Agent
# 模式：Prompt + LLM + Tools -> Runnable -> Assistant 包装器
# =============================================================================

def create_full_scan_agent(llm: Runnable) -> Assistant:
    """股票池扫描 Agent：获取市场股票列表"""
...
def create_primary_assistant(llm: Runnable) -> Assistant:
    """
    主 Agent（路由器）：负责理解用户意图并分发给子 Agent
    特点：不直接执行工具，而是调用 ToXxxAssistant 触发状态切换
    """
    prompt = ChatPromptTemplate.from_messages(
        [
...
def create_entry_node(assistant_name: str, new_dialog_state: str) -> Callable:
    ...
    1. 创建 ToolMessage 通知子 Agent 接管
    2. 更新 dialog_state 标记当前阶段
    """
...
def route_primary_assistant(state: AgentState) -> Literal[
    ...
    """
    主 Agent 路由：判断用户意图，分发给对应的子 Agent
    - 检测到 ToXxxAssistant 工具调用 -> 切换到对应子 Agent
    - 无工具调用 -> 结束
    """
    route = tools_condition(state)
    if route == END:
...
# =============================================================================
# 核心：构建 LangGraph Agent 图
# 架构说明：
#   1. 主 Agent (primary_assistant) -> 路由器，理解用户意图
#   2. 5 个子 Agent -> 专业处理特定任务
#   3. 每个子 Agent 有自己的工具节点 -> 实际执行任务
# =============================================================================
...
def create_agent_graph() -> StateGraph:
    ...
    builder.add_node(
        ...
        create_tool_node_with_fallback(
            [get_stock_universe, get_stock_price_history, get_stock_price]
        ),
    )
